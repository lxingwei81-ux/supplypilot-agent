...
- 最大缺口从8,800降至6,800；
- 来源工厂重算后最大缺口为0；
- 但来源工厂期末库存仅800，低于安全库存1,000，形成新的缓冲不足风险；
- 因而2,000件方案不能直接放行，应将调拨量降至不超过1,800件，或由计划/物流对安全库存例外进行明确审批。

控制塔Hero采用更保守的1,800件调拨：目标工厂最大缺口降至7,000，来源工厂期末库存保留在安全库存1,000，且不新增缺料。两组数字对应不同方案，不应混用。
...
调拨不能只解决目标工厂问题，还必须证明来源工厂不会因此缺料或跌破必须保留的安全库存。
